...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [NMBR_LYR] = LAYOUT_split_3x5_2( // NUMBERS, SYMBOLS, ARROWS
        ...
        KC_AMPR, KC_ASTR, KC_LPRN, KC_RPRN, KC_TRNS,       KC_TRNS, KC_PERC, KC_WH_D, KC_WH_U, KC_CIRC,
                                               KC_TRNS, KC_TRNS,       KC_TRNS, KC_TRNS),

    [GAME_LYR] = LAYOUT_split_3x5_2( // NO COMBOS OTHER THAN LAYER SWITCHING -- APPROPRIATE FOR GAMING
        KC_1, KC_2, KC_3, KC_4, KC_5,                      KC_6, KC_7, KC_8, KC_9, KC_0,
        KC_A, KC_W, KC_S, KC_D, KC_Q,                      KC_E, KC_LEFT, KC_DOWN, KC_UP, KC_RGHT,
        KC_LEFT, KC_UP, KC_DOWN, KC_RGHT, KC_E,            KC_Q, KC_A, KC_S, KC_W, KC_D,
                                             KC_ESC, KC_L,             KC_R, KC_ENT)
};
...
const uint16_t PROGMEM cap_combo[] =   {KC_J, KC_L, COMBO_END};
...
combo_t key_combos[] = {
  ...
  [CAP_CMB] = COMBO(cap_combo, KC_CAPS),      [DFT_CMB] = COMBO(dft_combo, TO(DFLT_LYR)),
  // xooxo oxoox
...
// only layer-change-combos apply to the game layer
bool combo_should_trigger(uint16_t combo_index, combo_t *combo,
                          uint16_t keycode, keyrecord_t *record) {
    if (layer_state_is(GAME_LYR))
        if (combo_index != DFT_CMB && combo_index != MSE_CMB)
            return false;
    return true;
}
